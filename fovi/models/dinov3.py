...
def _remap_dinov3_layer_keys(
    ...
    """Translate known Transformers block paths before recursive strict loading.

    Rename the whole block prefix so ordinary weights and LoRA parametrizations
    move together. Unknown keys and tensor shape errors remain PyTorch errors.
    """
    layouts = ('layer', 'model.layer', 'encoder.layer')
    layers = _get_dinov3_layers(module)
    target = next(name for name, child in module.named_modules() if child is layers)
    if target not in layouts:
        raise RuntimeError(f'Unsupported DINOv3 layer path: {target!r}')
    sources = [
        layout for layout in layouts
        if any(key.startswith(f'{prefix}{layout}.') for key in state_dict)
    ]
    if len(sources) > 1:
        raise RuntimeError(f'Checkpoint contains multiple DINOv3 layer layouts: {sources}')
    if not sources or sources[0] == target:
        return
    source_prefix = f'{prefix}{sources[0]}.'
    target_prefix = f'{prefix}{target}.'
    for key in tuple(state_dict):
        if key.startswith(source_prefix):
            state_dict[target_prefix + key[len(source_prefix):]] = state_dict.pop(key)
...
def build_fovi_dinov3(cfg, device='cuda'):
    ...
    model.register_load_state_dict_pre_hook(_remap_dinov3_layer_keys)
